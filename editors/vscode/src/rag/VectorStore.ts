/**
 * VectorStore.ts
 * OmriCode — SQLite Vector Store for Embeddings
 *
 * Stores and searches document embeddings using SQLite with
 * sqlite-vec extension for vector similarity search.
 * Falls back to in-memory cosine similarity if sqlite-vec is unavailable.
 *
 * Used by: CodebaseRAG (file chunks) and CommentIndex (comment vectors).
 * Each entry stores: id, text, filePath, startLine, endLine, embedding (BLOB), timestamp.
 */
...
export class VectorStore {
   ...
   * Attempts to load sqlite-vec; falls back to in-memory if unavailable.
   */
  async initialize(): Promise<void> {
    try {
      ...
      // No sqlite available — use in-memory store only
      this.db = null;
    }
  }
  ...
  /**
   * Remove entries for a file path.
   */
  ...
  /**
   * Search entries by embedding vector (cosine similarity).
   * If no embedding provided, falls back to keyword search.
   */
...
/**
 * Simple Float32Array embedding from text using a local ONNX model
 * or a lightweight hash-based approach when no model is available.
 */
export class EmbeddingProvider {
  ...
  /**
   * Load the embedding model. Tries ONNX runtime first, then falls back.
   */
  async loadModel(): Promise<void> {
    try {
      // Try loading a small ONNX model (all-MiniLM-L6-v2)
  ...
  /**
   * Embed a text string into a Float32Array.
   */
  ...
  /**
   * Embed using the loaded ONNX model.
   */
  ...
  /**
   * Fallback hash-based embedding (deterministic, dimension=384).
   */
